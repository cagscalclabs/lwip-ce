...
These mechanisms are implemented via the \textbf{cryptoguard} wrapper detailed in the next section.
	...
	\IfFileExists{../../src/tls/core/share/crypto_guard.asm}{
		\inputminted[
			breaklines,
			breakanywhere,
			fontsize=\scriptsize
		]{tasm}{../../src/tls/core/share/crypto_guard.asm}
	}{
		The file \texttt{../../src/tls/core/share/crypto\_guard.asm} was not found at build time.
	}
...
When evaluated in unison, these mechanisms provide best-effort memory sanitization across allocation, use, and teardown paths, reducing residual plaintext/key exposure without claiming complete protection against a local or physical adversary.
	...
	\begin{table}[htbp]
		...
		{\footnotesize
			...
			x25519 & Peter Tillema (PT\_) \\\hline
	...
	This appendix includes the raw RNG dataset currently present in \texttt{datasets/appendix\_a} (relative to this \texttt{.tex} file at build time).
	
	\IfFileExists{datasets/appendix_a/entropy_runs.dat}{
		\inputminted[
			breaklines,
			breakanywhere,
			fontsize=\footnotesize
		]{text}{datasets/appendix_a/entropy_runs.dat}
	}{
		No files were found in \texttt{datasets/appendix\_a} at build time.
	}
