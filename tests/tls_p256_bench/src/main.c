...
/* Benchmark: Single ECDH key exchange */
int main(void) {
    os_ClrHome();
    ...
    /* Generate Bob's public key: Bob_pub = bob_private * G */
    ...
    if(!p256_ecdh(shared_secret, alice_private, &bob_public)) {
        printf("failed");
        os_GetKey();
        return 1;
    }
    ...
    /* Also verify Bob's public key is not at infinity */
    is_zero = true;
    for(int i = 0; i < 32; i++) {
        if(bob_public.x[i] != 0 || bob_public.y[i] != 0) {
            is_zero = false;
            break;
        }
    }
