...
vectors where CAVP coverage is unavailable, and emit:
  ...
  - vectors/CAVPIN.8xv     The calc-side input AppVar (binary, packed).
  - vectors/expected.json  The host-side grading source (test_id -> expected).

This script is the single source of truth for the CAVP test inputs. Neither
CAVPIN.8xv nor expected.json is committed to git; both are regenerated by
...
CAVPIN_PATH = VECTORS_DIR / "CAVPIN.8xv"
...
def write_8xv(body: bytes) -> None:
    """Wrap body bytes into an AppVar at CAVPIN_PATH. Uses convbin if
    available; falls back to a pure-Python packer."""
    CAVPIN_PATH.parent.mkdir(parents=True, exist_ok=True)
    if shutil.which("convbin"):
        ...
        try:
            subprocess.run(
                ["convbin", "-i", bin_path, "-o", str(CAVPIN_PATH),
...
def build_expected(vectors: list[dict[str, Any]], seed: int, sample_size: int) -> dict[str, Any]:
    ...
    return {
        "$comment": (
            "Generated by tests/common/scripts/cavp_fetch.py. Not committed to git. "
...
def main() -> int:
    ...
    try:
        ...
        print(f"==> ERROR packing CAVPIN.8xv: {e}", file=sys.stderr)
        return 2
    print(f"==> wrote {CAVPIN_PATH} ({CAVPIN_PATH.stat().st_size} bytes, "
