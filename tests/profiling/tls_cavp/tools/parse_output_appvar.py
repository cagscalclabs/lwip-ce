...
def main() -> int:
    ...
    md_lines.append("")
    md_lines.append(f"- Generated at: `{expected_data.get('generated_at', 'unknown')}`")
    md_lines.append(f"- CAVP fetch seed: `{expected_data.get('seed', 'unknown')}`")
    md_lines.append(f"- Sample size: `{expected_data.get('sample_size', 'unknown')}`")
    md_lines.append(f"- Expected vectors: `{len(vectors_by_id)}`")
    md_lines.append(f"- Calc responses: `{len(responses)}`")
    md_lines.append("")
