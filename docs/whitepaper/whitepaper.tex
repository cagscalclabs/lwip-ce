...
When evaluated in unison, these mechanisms provide best-effort memory sanitization across allocation, use, and teardown paths, reducing residual plaintext/key exposure without claiming complete protection against a local or physical adversary.
		...
		\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.22\textwidth}>{\raggedright\arraybackslash}p{0.17\textwidth}>{\raggedright\arraybackslash}p{0.55\textwidth}@{}}
			...
			& Planned & ECDSA-P-256, RSA keys up to 4096 bits where timing and memory permit, and wider RSA-PSS hash variants. \\
			& Out of scope & RSA-PKCS1 \texttt{CertificateVerify}, which TLS~1.3 forbids. \\
			\addlinespace
			...
			& Planned & None currently committed. \\
			& Out of scope & Client-certificate identity support and private-key signing on device. \\
			\addlinespace
			...
			& Planned & Warning-only truststore age checks once reliable time exists. \\
			& Out of scope & Treating AppVars or local storage as tamper-resistant. \\
			\addlinespace
			...
			& Planned & Append-only export growth after v1.0 with stable existing slot meanings. \\
			& Out of scope & Backward compatibility for calls to exports that do not exist in the installed resident app. \\
			\addlinespace
			...
			& Planned & Continued timing/profiling coverage and wider sample collection. \\
			& Out of scope & Secure boot, protected memory, secure key storage, and resistance to determined physical compromise. \\
			\bottomrule
		\end{longtable}
		\endgroup
