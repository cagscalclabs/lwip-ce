...
\end{table}
		...
		\end{itemize}
			...
			\begin{itemize}
				\item Approximately 8,192 bytes of static memory for: ARP tables, DNS resolver state, dispatch/timer states, RNG state, and more. During build of lwIP we force this data to live in the first 8 KB of the BSS and we force consumers to link their $BSSHEAP_LOW$ above that region for memory safety.
			...
			%% Outer table: Component | Description / nested data table
			%% Inner tables are unruled tabular with cols: Acc. Idle Typical Max Buffer
			\providecommand{\meminner}[1]{%
				\begin{tabular}[t]{@{}
					>{\small\centering\arraybackslash}p{0.04\textwidth}
					>{\small\raggedleft\arraybackslash}p{0.10\textwidth}
					>{\small\raggedleft\arraybackslash}p{0.10\textwidth}
					>{\small\raggedleft\arraybackslash}p{0.10\textwidth}
					>{\small\raggedright\arraybackslash}p{0.30\textwidth}@{}}
			...
			\begin{longtable}{>{\raggedright\arraybackslash}p{0.20\textwidth}>{\raggedright\arraybackslash}p{0.76\textwidth}}
				\toprule
				\textbf{Component} & \textbf{Description / Allocations} \\
				\midrule
			\endfirsthead
				\toprule
				\textbf{Component} & \textbf{Description / Allocations} \\
			...
			%% ── Accounting key ─────────────────────────────────────────────────
			Accounting key
				& \textbf{P} = packet buffer pool \quad
			...
			%% ── BSS link-time ──────────────────────────────────────────────────
			Link-time BSS reservation
				& Fixed at build; present from power-on; protected by \texttt{BSSHEAP\_LOW} \\
			\midrule
			&
			\meminner{%
				n/a & ${\sim}400\,\text{B}$   & ${\sim}400\,\text{B}$   & ${\sim}400\,\text{B}$   & \texttt{arp\_table} \\
				n/a & ${\sim}1{,}200\,\text{B}$ & ${\sim}1{,}200\,\text{B}$ & ${\sim}1{,}200\,\text{B}$ & \texttt{dns\_table} / \texttt{dns\_servers} / \texttt{dns\_pcbs} \\
				n/a & ${\sim}150\,\text{B}$   & ${\sim}150\,\text{B}$   & ${\sim}150\,\text{B}$   & \texttt{g\_dispatchers} \\
				n/a & ${\sim}100\,\text{B}$   & ${\sim}100\,\text{B}$   & ${\sim}100\,\text{B}$   & \texttt{g\_tls\_drbg\_state} / \texttt{g\_tls\_drbg\_seed\_material} \\
				n/a & $520\,\text{B}$         & $520\,\text{B}$         & $520\,\text{B}$         & \texttt{\_\_tls\_scratch} \\
				n/a & $256\,\text{B}$         & $256\,\text{B}$         & $256\,\text{B}$         & \texttt{\_\_rsa\_transient} \\
				n/a & ${\sim}80\,\text{B}$    & ${\sim}80\,\text{B}$    & ${\sim}80\,\text{B}$    & \texttt{g\_mem\_cfg} \\
				n/a & $<10\,\text{B}$         & $<10\,\text{B}$         & $<10\,\text{B}$         & \texttt{ifnums\_used} / \texttt{eth\_driver\_shutting\_down} / \texttt{g\_eth\_last\_rx\_activity\_ms} \\[4pt]
			\textbf{Total} & ${\sim}2{,}716\,\text{B}$ & ${\sim}2{,}716\,\text{B}$ & ${\sim}2{,}716\,\text{B}$ & \textit{fixed at link time} \\
			} \\
			\midrule

			%% ── @lwip_start
			%% @lwip_start allocations
			\texttt{lwip\_start()}
				& Heap allocations made during stack initialization; permanent for the lifetime of the stack \\
			\midrule
			&
			\meminner{%
			...
			\textbf{Total} & ${\sim}8{,}342\,\text{B}$ & ${\sim}8{,}342\,\text{B}$ & ${\sim}8{,}342\,\text{B}$ & \textit{permanent} \\
			} \\
			\midrule

			%% ── @eth_connect ───────────────────────────────────────────────────
			\texttt{eth\_device\_init()}
				& Allocated on USB device connect; freed after unplug once all in-flight transfers complete \\
			\midrule
			&
			\meminner{%
				H & $0$ & ${\sim}2{,}200\,\text{B}$ & ${\sim}2{,}200\,\text{B}$ & \texttt{eth\_device\_t} \\
				R & $0$ & $512\,\text{B}$           & $2{,}048\,\text{B}$       & RX overflow ring \\[4pt]
			\textbf{Total} & $0$ & ${\sim}2{,}712\,\text{B}$ & ${\sim}4{,}248\,\text{B}$ & \textit{on USB connect} \\
			} \\
			\midrule

			%% ── TCP connection ─────────────────────────────────────────────────
			TCP connection
				& Per open TCP connection; costs are additive \\
			\midrule
			&
			\meminner{%
				H & ${\sim}200\,\text{B}$ & ${\sim}200\,\text{B}$ & ${\sim}200\,\text{B}$ & \texttt{tcp\_pcb} \\
				H & ${\sim}63\,\text{B}$  & ${\sim}63\,\text{B}$  & ${\sim}63\,\text{B}$  & \texttt{dhcp} \\
			} \\
			\midrule

			%% ── altcp connection ───────────────────────────────────────────────
			altcp connection
				& Thin altcp shim; sits above a TCP PCB \\
			\midrule
			&
			\meminner{%
				H & ${\sim}25\,\text{B}$ & ${\sim}25\,\text{B}$ & ${\sim}25\,\text{B}$ & \texttt{altcp\_pcb} \\
			} \\
			\midrule

			%% ── altcp TLS connection ───────────────────────────────────────────
			altcp TLS connection
				& Per open TLS connection; transient buffers exist only during handshake or active record processing \\
			\midrule
			&
			\meminner{%
				...
				T & $0$                       & ${\sim}1$--$5\,\text{KiB}$ & $5\,\text{KiB}$           & cert walker DER buf \\
			} \\
			\midrule

			%% ── UDP connection ─────────────────────────────────────────────────
			UDP connection
				& Per open UDP endpoint \\
			\midrule
			&
			\meminner{%
				H & ${\sim}52\,\text{B}$ & ${\sim}52\,\text{B}$ & ${\sim}52\,\text{B}$ & \texttt{udp\_pcb} \\
			} \\
			\midrule

			%% ── RAW connection ─────────────────────────────────────────────────
			RAW connection
				& Per open raw IP endpoint \\
			\midrule
			&
			\meminner{%
				H & ${\sim}49\,\text{B}$ & ${\sim}49\,\text{B}$ & ${\sim}49\,\text{B}$ & \texttt{raw\_pcb} \\
			} \\
			\midrule

			%% ── Outbound TCP segments ──────────────────────────────────────────
			Outbound TCP segments
				& Segment copies queued for retransmission; cleared on ACK \\
			\midrule
			&
			\meminner{%
				H & $0$ & ${\sim}512\,\text{B}$ & $4\,\text{KiB}$ & TCP send queue (\texttt{TCP\_SND\_BUF}) \\
			} \\
			\midrule

			%% ── lwip_socket RX ring ─────────────────────────────────────────────
			\texttt{lwip\_socket} RX ring
				& Per open socket; elastic ring buffer for application-consumed RX data; grows on pressure, shrinks on drain \\
			\midrule
			&
			\meminner{%
				C & $512\,\text{B}$ & ${\sim}512\,\text{B}$--$2\,\text{KiB}$ & $4\,\text{KiB}$ & \texttt{lwip\_socket.rx\_ring} (\texttt{LWIP\_SOCKET\_RX\_RING\_MAX\_SIZE}) \\
			} \\
