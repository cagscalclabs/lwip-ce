...
\end{table}
		...
		In addition to the USB driver, a dedicated \texttt{membuffer} allocator layer was implemented to keep memory behavior predictable across lwIP, USB RX/TX staging, and TLS components. On this platform, allocator stability is a correctness property: when memory pressure spikes, transport behavior and security-sensitive state handling must degrade in a controlled way rather than fail unpredictably. The subsystem can run in \textbf{dynamic mode} (wrapping toolchain \texttt{malloc}/\texttt{free}/\texttt{realloc} with explicit heap accounting and pressure reporting) or \textbf{static mode} (a caller-provided arena with fixed bounds and no external allocator dependency after initialization).
		...
		Within the dynamic path, each buffer tracks its own usage and resizes independently. Growth is proactive: once occupancy crosses a configurable \texttt{grow\_threshold\_pct}, the buffer reallocates upward by a fixed \texttt{step} immediately, bounded by a per-buffer \texttt{max\_size} (excluding the heap, which is capable of consuming all of \texttt{freeMem}, but almost never does). Shrinking is deliberately more conservative and uses true hysteresis rather than a mirrored threshold. A buffer only becomes shrink-eligible once occupancy drops to or below \texttt{shrink\_threshold\_pct}, and any check above that threshold immediately resets eligibility to zero. Once eligible, the buffer must remain below threshold for a holding period (\texttt{shrink\_hold\_count} consecutive low-usage checks) before it actually shrinks by one \texttt{step}; each successive shrink doubles the holding period required for the next one (\texttt{base} $\ll$ \texttt{stage}, capped at $\texttt{base} \ll 15$), so a buffer that keeps shrinking demands longer and longer quiet periods before shrinking again, and any renewed pressure resets the stage counter to zero. \texttt{BUFFER\_LOCK\_SIZE} disables growth and shrinking entirely, pinning a buffer at its initial size.

		Each buffer also derives its own pressure level from current usage versus capacity, tiered \texttt{NONE} $\rightarrow$ \texttt{MILD} ($>$70\%) $\rightarrow$ \texttt{HIGH} ($>$85\%) $\rightarrow$ \texttt{SEVERE} ($>$90\%) $\rightarrow$ \texttt{CRITICAL} ($>$95\% or unable to grow further), and invokes a per-buffer callback whenever that level changes in either direction. A separate global pressure level is derived from total heap usage and is layered onto buffer-level notifications via a \texttt{MEM\_PRESSURE\_GLOBAL} flag, letting unrelated modules coordinate throttling off a single signal; buffers flagged \texttt{BUFFER\_USER\_ALLOC} (caller-owned memory carved out of the heap, not the stack's own traffic) are excluded from this global accounting. This pressure model is what couples directly to the Ethernet ingress path: RX ring buffers use drain callbacks and timer-driven budgets, while pressure callbacks adjust scheduling and throttle behavior under load, keeping link handling responsive without runaway allocation during bursts.

		From a security perspective, any buffer can be flagged with \texttt{BUFFER\_SECURE\_MODE} (always on for \texttt{MEM\_BUFFER\_FILE}), enabling automatic zeroization on pop, free, release, and destroy. This does not replace higher-layer cryptographic hygiene, but it materially reduces residual plaintext/key exposure in normal allocator lifecycles.

		By default, lwIP-CE is allowed to use all available free RAM, growing and shrinking buffers as traffic demands; most of the heap sits idle between bursts. An application that needs to guarantee its own headroom can call \texttt{mem\_reserve} with however many bytes it needs, which the allocator treats as permanently off-limits and will not grow into. The reservation can be adjusted at any time with \texttt{mem\_resize} and released entirely with \texttt{mem\_release}. The stack tracks memory in six buckets, visible on the example HUD: \textbf{P} (the fixed $8\,\text{KiB}$ pbuf pool), \textbf{T} (TLS scratch and session buffers), \textbf{H} (everything else the stack allocates --- protocol control blocks, driver state, RNG, outbound segments, DNS scratch), \textbf{R} (the USB RX ring), \textbf{C} (per-connection application RX rings), and \textbf{U} (the caller's reservation via \texttt{mem\_reserve}, which shrinks H's available ceiling by however many bytes are claimed). The HUD updates on a short polling interval, so a climbing H bucket is a real-time signal that something is holding onto heap longer than it should.\label{ssec:mem_accounting}
		...
		\end{itemize}
			...
			The lwIP-CE stack initializes by consuming only the needed resources:
			\begin{itemize}
			...
			At perfectly idle, this is all the stack consumes. The remainder of the available heap is free for consumption by the stack as pressure grows, up until the calculator runs out of memory. An API for users for reserve memory is available, which lets the caller own a slice of heap that lwIP can never try to grow into.
			...
			A single-TLS-connection deployment peaks at roughly $29\,\text{KiB}$ for a typical single-record Certificate path --- the $10.5\,\text{KiB}$ baseline, about $1\,\text{KiB}$ of per-connection TLS state, one decrypted record pbuf (up to ${\sim}16\,\text{KiB}$), and the shared RSA padding pool. Because the cert walker holds only one cert's DER ($1$--$5\,\text{KiB}$ typical) and frees it before the next is fetched, the old full-chain buffering peak of ${\sim}16\,\text{KiB}$ is eliminated. A fragmented Certificate message raises peak usage toward $34\,\text{KiB}$, since the record pbuf and the current cert's DER buffer briefly coexist during chain verification; the walker allocation is bounded by one cert's size and is freed the moment verification completes. Non-Certificate handshake messages that span TLS records use flat reassembly, but those messages are small (at most a few KiB for \texttt{NewSessionTicket}), keeping fragmented overhead well below the former ceiling. Demand-driven lwIP work (outbound TCP segments, queued encrypted pbufs) adds whatever the application's workload calls for, bounded by the global allocator cap. For multi-connection deployments, add roughly $1\,\text{KiB}$ per additional idle TLS connection and budget the transient record-pbuf and reassembly cost per concurrent handshake.
		...
		The project is backed by a deterministic unit test suite covering the security-critical cryptographic and parser paths. Tests run on \texttt{cemu-autotester}, a JSON-workflow based tool that executes a program and waits for certain conditions to be true. and are structured around explicit acceptance criteria; where policy requires fail-closed behavior, tests assert rejection rather than silent degradation. (TLS-specific timing and CAVP profiling are treated separately; see Section~\ref{ssec:profiling}.)

		The suite is dyload-driven: each test links against the curated, manifest-gated \texttt{lwip.lib} produced by the nightly release build, the same artifact a consumer application loads at runtime, rather than compiling TLS core sources directly from \texttt{src/}. This exercises the actual released ABI surface---a curation drop or manifest omission shows up as a build or link failure in the test itself---and bootstraps the allocator and stack state through the real public \texttt{lwip\_start()} entry point rather than a hand-rolled, test-only initialization path.

		Coverage includes truststore integrity verification, certificate-chain trust decisions, parser correctness for ASN.1/X.509/PKCS flows, and expected rejection behavior for malformed or invalid inputs; the cryptographic primitives underneath them (hashing, HMAC, HKDF, PBKDF2, AES, RSA-OAEP/PSS, X25519, Base64) are each covered by a dedicated unit. RNG behavior is deliberately excluded from this suite and validated separately against hardware captures, as described below. Tests are built from applicable RFC vectors where available (with RFC and section identifiers recorded in test metadata), and from implementation-directed cases where no official vectors exist. Network-facing behavior of the Ethernet/USB transport---ARP, ICMP, and TCP/UDP handling against live probe traffic---is exercised separately by the DAST harness described in Section~\ref{ssec:profiling}; there is currently no dedicated unit-level harness for the Ethernet RX driver or the \texttt{membuffer} allocator in isolation.
...
\end{center}
	...
	Test execution is structured for reviewability rather than just pass/fail signaling. \texttt{build.yml} is the orchestrator: it builds the dylib once, then fans out to four self-audit workflows as parallel \texttt{workflow\_call} jobs --- the deterministic unit suite (\texttt{tests.yml}), CAVP-vector validation (\texttt{cavp.yml}), the differential timing profiler (\texttt{timing.yml}), and static analysis (\texttt{sast.yml}), alongside the DAST harness (\texttt{dast.yml}) --- each producing a self-contained job summary intended to be readable as an audit artifact without reference to source. The nightly \texttt{publish} job is itself gated on every one of these checks passing, so a release is never cut from a build that failed validation.
