...
\end{table}
		...
		This is directly coupled to the Ethernet ingress path: RX ring buffers use drain callbacks and timer-driven budgets, while pressure callbacks adjust scheduling/throttle behavior under load. In practice, this allows the stack to keep link handling responsive while avoiding runaway allocation during bursts.
			...
			%% ── @lwip_init ─────────────────────────────────────────────────────
			\texttt{lwip\_init()}
				& Heap allocations made during stack initialisation; permanent for the lifetime of the stack \\
			\midrule
			&
			\meminner{%
			...
			Two constraints govern all downstream processing of that plaintext buffer:
			...
			\subsubsection{Memory Ownership and the User Reservation API}%
			\label{ssec:mem_accounting}

			By default, lwIP-CE is allowed to use all available free RAM. It rarely needs all of it --- most of the heap sits idle between bursts of traffic. The idea is that your application should be able to take what it needs without fighting the stack for space.

			To do that, call \texttt{mem\_reserve} with however many bytes your app needs. lwIP will treat that region as off-limits and will not grow into it. You can adjust the reservation at any time with \texttt{mem\_resize}, and release it entirely with \texttt{mem\_release} when you're done.

			The stack tracks memory in six buckets, visible on the example HUD:

			\begin{itemize}
				\item \textbf{P} --- the pbuf pool; fixed at $8\,\text{KiB}$ for in-flight frame data.
				\item \textbf{T} --- TLS scratch and session buffers.
				\item \textbf{H} --- everything else the stack allocates: protocol control blocks, driver state, RNG, outbound segments, DNS scratch, etc. If an allocation doesn't belong to another bucket, it lands here.
				\item \textbf{R} --- the USB RX ring.
				\item \textbf{C} --- per-connection application RX rings.
				\item \textbf{U} --- your reservation. This shows how many bytes you've claimed via \texttt{mem\_reserve}. When non-zero, H's available ceiling shrinks by this amount.
			\end{itemize}

			The HUD updates on a short polling interval so you can watch allocations rise and fall in real time. If H is climbing unexpectedly, that's your signal that something is holding onto heap longer than it should.
	...
	\paragraph{A note on the earlier SPKI-pinning model}
	Earlier revisions expressed chain trust as signed \texttt{SubjectPublicKeyInfo} (SPKI) pins: the chain was accepted if a CA-capable certificate in it matched a pinned digest in a shipped truststore. That mechanism has been \emph{replaced} by the adjacent-link verification described below. A signed truststore is still generated and shipped (its format and lifecycle are detailed in the next section) and remains the foundation for a future issuer-key/root-anchoring scheme, but it is no longer the gate for chain acceptance in this release.
	...
	\begin{itemize}
		...
		\item Any other signature type---\texttt{ecdsa\_secp256r1\_sha256}, wider RSA-PSS hashes, or a non-RSA issuer key the link cannot be checked against---is \emph{not} verified. The link is accepted and the stack raises an \texttt{ALERT}-severity diagnostic (``unsupported cert type, proceeding'') so the application can observe that the link went cryptographically unchecked.
		\item The topmost certificate has no issuer within the presented chain and is left unverified; this release does not anchor the top link to a truststore root.
	\end{itemize}
	The handshake therefore completes when every RSA-2048 link it could verify did verify, no verifiable link failed, and the leaf's \texttt{CertificateVerify} signature checks out. This is an interim posture: the \texttt{ALERT}-but-accept fallback for unsupported signature algorithms exists only until those algorithms are implemented (\texttt{ecdsa\_secp256r1\_sha256} first) and a truststore-anchored root check is added for the top of the chain. The on-device truststore format is specified in the next section.
