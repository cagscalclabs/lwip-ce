			...
			\begin{figure}[H]
				...
				\begin{tikzpicture}[
					...
					\node[lib, below=of find] (verify) {\textbf{Verify} \texttt{"LWIPTB"} \textbf{magic}\\
						count/magic mismatch $\Rightarrow$ fail closed (stale/incompatible app)};
					\node[lib, below=of verify] (patch) {\textbf{Patch each trampoline} $\rightarrow$ relocated in-app address\\
