...
\end{table}
		...
		\end{itemize}
			...
			\multicolumn{1}{l}{\textbf{Subtotal}} & $3{,}528\,\text{B}$ & \textit{\texttt{.bss}, fixed at link time (\texttt{\_\_bss\_len})} \\[6pt]
			...
			\multicolumn{1}{l}{\textbf{Subtotal}} & $616\,\text{B}$ & \textit{\texttt{.data}, fixed at link time (\texttt{\_\_data\_len})} \\[6pt]
			...
			\begin{meminner}
				...
				\rowcolor{gray!18}
			\multicolumn{1}{l}{\textbf{Subtotal}} & ${\sim}8{,}342\,\text{B}$ & ${\sim}8{,}342\,\text{B}$ & ${\sim}8{,}342\,\text{B}$ & \textit{\texttt{lwip\_start()}, stack up, idle} \\[6pt]
				H & $0$ & ${\sim}2{,}200\,\text{B}$ & ${\sim}2{,}200\,\text{B}$ & \texttt{eth\_device\_t} \\
				R & $0$ & $512\,\text{B}$           & $2{,}048\,\text{B}$       & RX overflow ring \\[4pt]
				\rowcolor{gray!18}
			\multicolumn{1}{l}{\textbf{Subtotal}} & $0$ & ${\sim}2{,}712\,\text{B}$ & ${\sim}4{,}248\,\text{B}$ & \textit{\texttt{eth\_device\_init()}, per device} \\[6pt]
				H & ${\sim}200\,\text{B}$ & ${\sim}200\,\text{B}$ & ${\sim}200\,\text{B}$ & \texttt{tcp\_pcb} \\
				H & ${\sim}63\,\text{B}$  & ${\sim}63\,\text{B}$  & ${\sim}63\,\text{B}$  & \texttt{dhcp} \\[4pt]
				\rowcolor{gray!18}
			\multicolumn{1}{l}{\textbf{Subtotal}} & ${\sim}263\,\text{B}$ & ${\sim}263\,\text{B}$ & ${\sim}263\,\text{B}$ & \textit{TCP connection, additive per connection} \\[6pt]
				H & ${\sim}25\,\text{B}$ & ${\sim}25\,\text{B}$ & ${\sim}25\,\text{B}$ & \texttt{altcp\_pcb} \\[4pt]
				\rowcolor{gray!18}
			\multicolumn{1}{l}{\textbf{Subtotal}} & ${\sim}25\,\text{B}$ & ${\sim}25\,\text{B}$ & ${\sim}25\,\text{B}$ & \textit{altcp connection, sits above a TCP PCB} \\[6pt]
				...
				\rowcolor{gray!18}
			\multicolumn{1}{l}{\textbf{Subtotal}} & ${\sim}1{,}110\,\text{B}$ & see below & ${\sim}34\,\text{KiB}$ & \textit{altcp TLS connection; buffers are not simultaneous, see peak figures below} \\[6pt]
				H & ${\sim}52\,\text{B}$ & ${\sim}52\,\text{B}$ & ${\sim}52\,\text{B}$ & \texttt{udp\_pcb} \\[4pt]
				\rowcolor{gray!18}
			\multicolumn{1}{l}{\textbf{Subtotal}} & ${\sim}52\,\text{B}$ & ${\sim}52\,\text{B}$ & ${\sim}52\,\text{B}$ & \textit{UDP connection, per endpoint} \\[6pt]
			...
			\multicolumn{1}{l}{\textbf{Subtotal}} & ${\sim}49\,\text{B}$ & ${\sim}49\,\text{B}$ & ${\sim}49\,\text{B}$ & \textit{RAW connection, per endpoint} \\[6pt]
				...
				\rowcolor{gray!18}
			\multicolumn{1}{l}{\textbf{Subtotal}} & $0$ & ${\sim}512\,\text{B}$ & $4\,\text{KiB}$ & \textit{outbound TCP segments, cleared on ACK} \\[6pt]
				C & $512\,\text{B}$ & ${\sim}512\,\text{B}$--$2\,\text{KiB}$ & $4\,\text{KiB}$ & \texttt{lwip\_socket.rx\_ring} (\texttt{LWIP\_SOCKET\_RX\_RING\_MAX\_SIZE}) \\[4pt]
				\rowcolor{gray!18}
			\multicolumn{1}{l}{\textbf{Subtotal}} & $512\,\text{B}$ & ${\sim}512\,\text{B}$--$2\,\text{KiB}$ & $4\,\text{KiB}$ & \textit{\texttt{lwip\_socket} RX ring, per open socket} \\
			\end{meminner}
...
for (uint8_t i = 0; i < count; i++) {
	...
	\begin{tabularx}{0.94\textwidth}{@{}p{0.25\textwidth}X@{}}
		...
		\midrule
		Supported now & TLS~1.3 client handshakes, RSA-PSS/SHA-256 \texttt{CertificateVerify} for 1024--2048 bit RSA leaf keys, streaming certificate-chain parsing, RSA/SHA-256 adjacent-link checks where supported, empty client-certificate response handling, signed compact truststore loading, and application-visible warnings when trust anchoring is incomplete. \\
		Planned & ECDSA-P-256 verification, RSA keys up to 4096 bits where performance permits, wider RSA-PSS hash variants, and non-fatal truststore age warnings once a reliable time source is available. \\
	...
	The handshake therefore completes when every RSA link it could verify---including the root anchoring check---did verify, no verifiable link failed, and the leaf's \texttt{CertificateVerify} signature checks out. The on-device truststore format is specified in the next section.
...
When evaluated in unison, these mechanisms provide best-effort memory sanitization across allocation, use, and teardown paths, reducing residual plaintext/key exposure without claiming complete protection against a local or physical adversary.
		...
		\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.22\textwidth}>{\raggedright\arraybackslash}p{0.17\textwidth}>{\raggedright\arraybackslash}p{0.55\textwidth}@{}}
			...
			TLS Certificate & Supported now & RSA/SHA-256 adjacent-link verification, fail-closed. \\
